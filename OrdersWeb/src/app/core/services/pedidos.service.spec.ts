...
// Interfaz que coincide con la estructura de respuesta de la API
export interface PaginatedResponse<T> {
  ...
  enlace_pagina_anterior?: string;
  enlace_pagina_siguiente?: string;
...
export class PedidosService {
  ...
  obtenerTodos(pagina: number = 1, tamano: number = 10): Observable<PaginatedResponse<Pedido>> {
    ...
    return this.http.get<PaginatedResponse<Pedido>>(this.baseUrl, { 
      ...this.getHttpOptions(),
      params 
    });
  }
  ...
  cambiarEstado(id: number, nuevoEstado: string, rutaId?: number): Observable<Pedido> {
    const body: any = { estado_pedido: nuevoEstado };
    if (rutaId) {
      body.ruta_id = rutaId;
    }
